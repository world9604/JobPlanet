...
class SalaryViewHolder(val itemBinding : ItemSalaryBinding, val viewModel: CompanyListViewModel)
    ...
    val userActionListener = object: CellTypeItemUserActionListener {
        override fun onCellTypeItemClicked(companyId: Int) {
            viewModel.openCellTypeItem(companyId, CompanyCellType.CELL_TYPE_SALARY)
        }
    }
}